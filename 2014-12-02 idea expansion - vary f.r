...
sim <- function(beta1=1, f=f, n_rep=1000){
  #parameter
  beta0=110; beta1=beta1; var=5; f=f; n_family=1000; n_pop=2*n_family
  n_rep=n_rep
  ...
  sim.result <- replicate(n_rep, {
    ...
    fit_decomp <- optim(beta<-c(110,1,5), fn_decomp, data=generated_family, hessian=T, method="L-BFGS-B",,lower=c(-Inf,-Inf, 0))
    ...
    #     n_info_family <- nrow(generated_family_informative)
    #     IBDshare <- with(generated_family_informative, ifelse(n_IBD_carrier>=1 & n_nonIBD_carrier==0, "S", "NS"))
    #     #   (with(generated_family_informative, tapply(sib_pheno_sum, IBDshare, mean)))
    #     (shareornot.test.p.value <- t.test(sib_pheno_sum~IBDshare, data=generated_family_informative, var.equal=T)$p.value)
    #     
    #top 50% sibpairs
