...
sim_result <- replicate(n_rep, {
  ...
  data.cc <- gene_case_control_pe(n_case_control_pair=n_cc, trait_mean=0, Beta=Beta, dis_cutoff = NA)
...
opts["param"] = "--time=1-0:0 --exclude=c06" #indicate this is a quick job
######################
...
opts['dis_cutoff']='1.281' #prevalence
opts['f'] = 0.01 #rare
for i in numpy.linspace(0,1,num=15):
	for j in range(opts['n_ite']):
		opts['r'] = i
...
result <- read.csv("2015-12-2. correct trap and trafic_ext compare with pedgene, cc, and fbat_ext 2g2c_at p_dis0.1.csv", header=T)
result <- result %>% gather(key="method", value="p.value", 9:15)
levels(result$method)[7] <- "result.pop"
...
filter(result.plot, !grepl("vc|ind", method)) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
