...
sim_result <- replicate(n_rep, {
  ...
  result.trap.ind.lm.lm.only.only.founder <- temp.trap.ind.lm$p.value.only.founder
  result.trap.ind.lm.lm.only.only.founder.SKAT <- temp.trap.ind.lm$p.value.only.founder.SKAT 
  
  #pedgene with full samples
  ...
  c(result.trap.ind,
    ...
    result.trap.ind.lm.lm.only,
  	result.trap.ind.lm.lm.only.only.founder,
  	result.trap.ind.lm.lm.only.only.founder.SKAT,
...
result <- read.csv("2016-01-19 look further into lm part.csv", header=T)
result <- result %>% gather(key="method", value="p.value", 10:17)
...
#correlation plot between trap and lm
result <- read.csv("allResults.csv", header=T)
with(filter(result, family_strct=="2g.2f.2c"), plot(result.trap.ind.lm.trap.only, result.trap.ind.lm.lm.only))
with(filter(result, family_strct=="2g.2f.2c"), cor(result.trap.ind.lm.trap.only, result.trap.ind.lm.lm.only))
