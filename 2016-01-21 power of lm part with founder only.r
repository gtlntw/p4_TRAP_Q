...
#   result.pedgene.burden <- result.pedgene$pgdf$pval.burden
  ...
  c(result.trap.ind,
    ...
    result.trap.ind.lm.lm.only,
  	result.trap.ind.only.founder,
  	result.trap.ind.only.founder.SKAT)
})
...
result <- read.csv("2016-01-21 power of lm part with founder only.csv", header=T)
...
filter(result.plot, grepl("trap", method), !grepl("vc|hap|SKAT|lm.only|trap.only|prop", method)) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
