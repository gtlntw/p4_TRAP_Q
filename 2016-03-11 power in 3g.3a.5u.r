...
opts["param"] = "--time=1-12:0  --mem=6000 {exclude}".format(**opts) #indicate this is a quick job
######################
...
result <- read.csv("2016-03-11 power in 3g.3a.5u.csv", header=T)
result <- result %>% gather(key="method", value="p.value", 10:19)
result.plot <- result %>% group_by(family_strct, trait_mean, dis_cutoff,f, risk.haplo.f, r, method, exact_affected) %>% 
  summarise(n=n(), power=mean(p.value<0.05, na.rm=T))
result.plot$dis_cutoff[which(is.na(result.plot$dis_cutoff))] <- "NA"
result.plot$dis_cutoff <- factor(result.plot$dis_cutoff, labels=c("3g.3a.5u","non-ascertained"))
...
filter(result.plot, !grepl("vc|hap", method)) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
  ...
  facet_grid(dis_cutoff*exact_affected~trait_mean*centered, scale="free_x", labeller = label_both) +
