...
for(i in 1:n_rep) {
  ...
  result.trap.new <- family.rank.test(data=family_generated, f=risk.variant.id)$p.value
  result.trap.new.rank <- family.rank.test(data=family_generated, f=risk.variant.id)$p.value.rank
  sim.fail <- tryCatch({  
...
#   result.fbskat.burden=1-pchisq(x[,4],x[,5])
  ...
  #run tests 
  result.trap.centered <- family.test(data=family_generated_centered, f=risk.variant.id, summary.stat="3")$p.value
  result.trap.new.centered <- family.rank.test(data=family_generated_centered, f=risk.variant.id)$p.value
  result.trap.new.rank.centered <- family.rank.test(data=family_generated_centered, f=risk.variant.id)$p.value.rank
...
#   result.fbskat.burden.centered <- 1-pchisq(x[,4],x[,5])
  ...
  if(sim.fail==F) {
    ...
    sim_result[[i]] <- data.frame(result.trap, result.trap.new, result.trap.new.rank,
                                  ...
                                  result.pop,
                                  result.trap.centered, result.trap.new.centered, result.trap.new.rank.centered,
...
result <- read.csv("2016-03-13 new rank_based summary statistics.csv", header=T)
