...
#   result.fbskat.burden=1-pchisq(x[,4],x[,5])
  ...
  result.trap.new.version.centered <- family.rank.test(data=family_generated_centered, f=risk.variant.id)
  result.trap.new.centered <- result.trap.new.version.centered$p.value
  result.trap.new.rank.centered <- result.trap.new.version.centered$p.value.rank
...
#   result.fbskat.burden.centered <- 1-pchisq(x[,4],x[,5])
  ...
  if(sim.fail==F) {
    #only report p.value
    sim_result[[i]] <- data.frame(result.trap, result.trap.centered, result.trap.new,result.trap.new.centered, result.trap.new.rank,result.trap.new.rank.centered,
                                  result.pedgene.vc,result.pedgene.vc.centered, result.pedgene.burden,result.pedgene.burden.centered,
                                  # result.fbskat.vc, result.fbskat.burden,
                                  result.pop,result.pop.centered) 
  } else {
