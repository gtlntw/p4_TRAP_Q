...
#   result.fbskat.burden=1-pchisq(x[,4],x[,5])
  ...
  if(sim.fail==F) {
    #only report p.value
    sim_result[[i]] <- data.frame(result.trap, result.trap.new, result.trap.new.rank,
                                  result.pedgene.vc, result.pedgene.burden,
    															result.pedgene.vc_info_family, result.pedgene.burden_info_family,
...
#variance as a function of trait_mean
x <- c(-5, -1, 0 , 1, 5)
y <- c(48.93, 5.60, 4.63, 7.44, 60.54)
ggplot(data=data.frame(x=x, y=y), aes(x, y)) + 
	geom_point() +
	geom_line() +
	labs(x="trait_mean", y="mean_variance") +
	theme_gray(base_size = 20) +
  theme(legend.position="bottom", panel.background = element_rect(fill = 'grey85')) +
  scale_color_manual(values=cbbPalette)
