...
for(i in 1:n_rep) {
  ...
  result.trap_within_center_comb <- family.score.test(data=family_generated, f=risk.variant.id, summary.stat="5", lm.test=T, within_center = T)
  ...
  if(sim.fail==F) {
    #only report p.value
    sim_result[[i]] <- data.frame(result.trap_within_center_comb$p.value.comb, result.trap_within_center_comb$within.info, result.trap_within_center_comb$between.info) 
  } else {
...
result <- read.csv("2016-05-23 test independence within and between info.csv", header=T)
# result <- result %>% gather(key="method", value="p.value", -c(1:10))
result.plot <- result %>% group_by(family_strct, trait_mean, dis_cutoff,f, risk.haplo.f, r) %>% 
  summarise(n=n(), corr=cor(result.trap_within_center_comb.within.info, result.trap_within_center_comb.between.info))
...
#all f=0.01 and f=0.20 correlation r 
pd <- position_dodge(0.0)
filter(result.plot) %>% ggplot(aes(x=r, y=corr)) +
  #   geom_point(size=3, alpha=1) +
  facet_grid(dis_cutoff~f*family_strct, labeller = label_both) +
...
#all f=0.20
pd <- position_dodge(0.0)
filter(result.plot, f==0.20) %>% ggplot(aes(x=r, y=corr)) +
  #   geom_point(size=3, alpha=1) +
  facet_grid(dis_cutoff~trait_mean*f*family_strct, labeller = label_both) +
  geom_hline(yintercept = 0) +
  geom_line(size=1, alpha=0.8, position=pd) +
  geom_point(size=1.5, position=pd) +
#   ggtitle("f=0.202, consider effect size of risk haplotypes, TRAP") +
