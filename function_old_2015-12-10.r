...
family.test <- function(data=family_generated, f=risk.variant.id, nofounderphenotype=F, conditional.test=F, association.test=F, between.test=F, noIBD=F, summary.stat="1") {
...
#     )
  ...
  if(between.test==T) { #fisher's method to combine two p-values
    #association test using founder treadted as all affected 
		founder.idx <- which(data$data_family$father==0 & data$data_family$mother==0)
  	h1 <- data$data_family[founder.idx,7:(6+n_snp)]
    h2 <- data$data_family[founder.idx,-c(1:(6+n_snp))]
   	carrier <- apply(cbind(h1[, snp2look.idx], h2[, snp2look.idx]), 1, function(x) sum(x==1)>0) 
   	lm.result <- lm(data$data_family[founder.idx, "affect"] ~ carrier) #linear regression test
  	p.value.between <- summary(lm.result)$coefficient[2,4]
